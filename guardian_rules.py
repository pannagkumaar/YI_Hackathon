...
# ---------------------------------------------------------------------------
ALLOWED_TOOLS = {"run_script", "fetch_data", "restart_service", "list"}
...
def parse_proposed_action(proposed_action: str) -> Tuple[bool, Dict[str, Any], str]:
    ...
    if len(parts) != 2:
        # NEW tolerant fallback for natural language commands
        # Example: "list logs" -> {"action":"list","action_input":{"target":"logs"}}
        words = snippet.split()
        if len(words) == 1:
            return True, {"action": words[0], "action_input": {}}, ""
        if len(words) >= 2:
            return True, {"action": words[0], "action_input": {"target": " ".join(words[1:])}}, ""
        return False, {}, "Could not parse proposed_action"
...
# ---------------------------------------------------------------------------
def deterministic_eval_action(proposed_action: str, context: dict, policies: list) -> dict:
    """
    Deterministically evaluate a single proposed action against explicit disallow rules.
    Returns a structured dict compatible with Guardian's analyzer layer.
    """
    ...
    normalized = proposed_action.lower().strip()
    matched_policy = None

    # Default assumption — safe unless matched
    decision = "Allow"
    reason = "No violation detected."
    score = 0.0

    for rule in policies or []:
        keyword = rule.split(":", 1)[-1].strip().lower()
        if keyword and keyword in normalized:
            matched_policy = rule
            decision = "Deny"
            reason = f"Action violates policy: {rule}"
            score = 1.0
            break

    # Return a full structure — Guardian relies on these exact keys
    return {
        "decision": decision,
        "reason": reason,
        "evidence": {"policy": matched_policy or "None"},
        "policy_score": score,
        "requires_human_review": False if decision == "Allow" else True
    }
