...
app = FastAPI(title="Manager Service", description="Orchestrator for SHIVA.")
...
def discover(service_name: str) -> str:
    ...
    try:
        r = requests.get(f"{DIRECTORY_URL}/discover", params={"service_name": service_name})
...
def invoke(request: InvokeRequest):
    ...
    try:
        ...
        overseer_url = discover("overseer-service")
        status_resp = requests.get(f"{overseer_url}/control/status")
        ...
        # A real implementation would loop through all steps
        ...
        if partner_result.get("status") == "DEVIATION_DETECTED":
            # Here you would trigger /replan or /approve logic
